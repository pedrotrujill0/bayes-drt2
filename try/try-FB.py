...
Z_file_tp = os.path.join(datadir,'EFB','1.6V_1.xlsx')
Zdf_tp = pd.read_excel(Z_file_tp)

# extract frequency and complex impedance
freq_tp = Zdf_tp['f'].values
Z_tp = Zdf_tp['Re'].values + 1j*Zdf_tp['Im'].values

Zdf_tp = Zdf_tp.rename(columns=({'f':'Freq', 'Re':'Zreal','Im':'Zimag'}))
Zdf_tp['Zphz'] = np.arctan(Zdf_tp['Zreal']/Zdf_tp['Zimag']) 

"Load blocking planar data"
# load simulated impedance data with noise
Z_file_bp = os.path.join(datadir,'EFB','1.6V_1.xlsx')
Zdf_bp = pd.read_excel(Z_file_bp)

# extract frequency and complex impedance
freq_bp = Zdf_bp['f'].values
Z_bp = Zdf_bp['Re'].values + 1j*Zdf_bp['Im'].values

Zdf_bp = Zdf_bp.rename(columns=({'f':'Freq', 'Re':'Zreal','Im':'Zimag'}))
Zdf_bp['Zphz'] = np.arctan(Zdf_bp['Zreal']/Zdf_bp['Zimag']) 
...
def plot_result(inv, Zdf, tau, plot_ci = False, c = 'k', label = None, axes = None, plot_data = True):
    ...
    # plot the true DDT
    # gamma = inv.predict_distribution
    # if plot_data:
    #     p = axes[1,1].plot(tau, g_true['gamma'], label = 'True', ls = '--')
    #     # plot data limits
    ...
    g_pred = inv.predict_distribution(dist_name, tau)
    # plot the recovered DDT
    if plot_ci:
        # plot mean and 95% credibility interval (CI) if HMC sampling used
        g_lo = inv.predict_distribution(dist_name, tau, percentile=2.5)
        g_hi = inv.predict_distribution(dist_name, tau, percentile=97.5)
        axes[1,1].plot(tau, g_pred, c = c, label = 'Posterior mean', alpha=0.8)
        axes[1,1].fill_between(tau, g_lo, g_hi, color = c, label = '95% CI', alpha = 0.15)
    else:
        # otherwise plot point estimate only
        axes[1,1].plot(tau, g_pred, c = c, label = g_label, alpha=0.8)
...
axes = plot_result(inv_tp, Zdf_tp, tau_pred, plot_ci = True, label = 'HMC')
# overlay the MAP results
axes = plot_result(inv_tpm, Zdf_tp, tau_pred, plot_data = False, axes = axes, label = 'MAP', c = 'r')
...
axes = plot_result(inv_bp, Zdf_bp, tau_pred, plot_ci = True, label = 'HMC')
# overlay the MAP results
axes = plot_result(inv_bpm, Zdf_bp, tau_pred, plot_data = False, axes = axes, label = 'MAP', c = 'r')
...
# for name in inv_tp.__dict__.keys():
#     print(name)